...
func (self *ZeroCopySource) NextFixed64() (data Fixed64, eof bool) {
	var val int64
	val, eof = self.NextInt64()
	return Fixed64(val), eof
}
...
func (self *ZeroCopySource) NextHash() (data Uint256, eof bool) {
	var buf []byte
	buf, eof = self.NextBytes(UINT256SIZE)
	if eof {
		return
	}
	copy(data[:], buf)
...
func (self *ZeroCopySource) NextUint160() (data Uint160, eof bool) {
	var buf []byte
	buf, eof = self.NextBytes(UINT160SIZE)
	if eof {
		return
	}
	copy(data[:], buf)
